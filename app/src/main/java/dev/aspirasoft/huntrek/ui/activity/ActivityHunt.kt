...
import dev.aspirasoft.huntrek.data.repo.AuthRepository.signIn
import dev.aspirasoft.huntrek.data.repo.TreasureRepository.all
import dev.aspirasoft.huntrek.data.repo.TreasureRepository.get
import dev.aspirasoft.huntrek.data.repo.TreasureRepository.pop
import dev.aspirasoft.huntrek.data.repo.TreasureRepository.size
import dev.aspirasoft.huntrek.listener.TimeListener
import dev.aspirasoft.huntrek.model.Game
import dev.aspirasoft.huntrek.model.collectibles.TreasureChest
import dev.aspirasoft.huntrek.ui.dialog.DialogGameMenu
import dev.aspirasoft.huntrek.ui.dialog.DialogUserDetails
import dev.aspirasoft.huntrek.ui.view.AvatarView
import dev.aspirasoft.huntrek.ui.view.GameStartingView
import dev.aspirasoft.huntrek.utils.SpawnUtil.checkDaySinceLastSpawn
import dev.aspirasoft.huntrek.utils.SpawnUtil.spawnTreasureChests
...
class ActivityHunt : AppCompatActivity(), View.OnClickListener, OnMapReadyCallback, OnMarkerClickListener,
    ...
    private fun collectTreasure(id: Int) {
        val chest = pop(id)
        if (chest != null) {
            Log.i(dev.aspirasoft.huntrek.HuntItApp.TAG, "Received " + chest.value + " coins from chest # " + id)
            mGame.player.addPoints(chest.value)
            mGame.player.info.chestsOpened++
        } else {
            Log.e(dev.aspirasoft.huntrek.HuntItApp.TAG, "Could not read chest # $id")
        }
    }
    ...
    private inner class GameInitializer : GameStartListener {
        ...
        private fun showPaused() {
            mGameMap.moveCamera(CameraUpdateFactory.newCameraPosition(CameraPosition.Builder()
                .target(dev.aspirasoft.huntrek.data.HuntBoundary.CENTER)
                .zoom(14.0)
                .tilt(0.0)
                .build()))
        }
