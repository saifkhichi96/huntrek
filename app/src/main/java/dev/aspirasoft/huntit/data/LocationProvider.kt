...
import dev.aspirasoft.huntit.data.LocationProvider.LocationProvider
...
class LocationProvider(context: Context) {
    ...
    private fun registerProvider(context: Context, provider: String) {
        ...
        manager.requestLocationUpdates(provider,
            dev.aspirasoft.huntit.data.LocationProvider.Companion.REFRESH_DELAY,
            dev.aspirasoft.huntit.data.LocationProvider.Companion.REFRESH_DISTANCE, object : LocationListener {
                /**
    ...
    private fun enableProvider(provider: String) {
        try {
            ...
            Log.i(dev.aspirasoft.huntit.data.LocationProvider.Companion.TAG, "Enabled $provider location provider.")
        } catch (ex: Exception) {
    ...
    private fun disableProvider(provider: String) {
        ...
        Log.i(dev.aspirasoft.huntit.data.LocationProvider.Companion.TAG, "Disabled $provider location provider.")
    ...
    private fun autoSwitchProviders() {
        ...
        if (bestProvider != null && bestProvider.name != currentProvider) {
            Log.i(dev.aspirasoft.huntit.data.LocationProvider.Companion.TAG,
