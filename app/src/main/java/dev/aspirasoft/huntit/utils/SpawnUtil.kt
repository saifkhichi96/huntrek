...
import dev.aspirasoft.huntit.data.HuntBoundary
import dev.aspirasoft.huntit.data.repo.TreasureRepository
    ...
    fun spawnTreasureChests() {
        for (i in 0 until MAX_CHESTS) {
            ...
            if (Random().nextInt(2) == 0) { // P = (1 - sqrt(r1)) * A + (sqrt(r1) * (1 - r2)) * B + (sqrt(r1) * r2) * C
                latitude =
                    (1 - sqrt(r1)) * HuntBoundary.POINT_A.x + sqrt(r1) * (1 - r2) * HuntBoundary.POINT_B.x + sqrt(
                        r1) * r2 * HuntBoundary.POINT_C.x
                longitude =
                    (1 - sqrt(r1)) * HuntBoundary.POINT_A.y + sqrt(r1) * (1 - r2) * HuntBoundary.POINT_B.y + sqrt(
                        r1) * r2 * HuntBoundary.POINT_C.y
            } else {
                latitude =
                    (1 - sqrt(r1)) * HuntBoundary.POINT_A.x + sqrt(r1) * (1 - r2) * HuntBoundary.POINT_D.x + sqrt(
                        r1) * r2 * HuntBoundary.POINT_C.x
                longitude =
                    (1 - sqrt(r1)) * HuntBoundary.POINT_A.y + sqrt(r1) * (1 - r2) * HuntBoundary.POINT_D.y + sqrt(
                        r1) * r2 * HuntBoundary.POINT_C.y
            }
